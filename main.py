...
def login():
    ...
    if login_type == '1':  # Admin
        ...
        user = conn.execute('SELECT * FROM faculty_list WHERE email = ? AND password = ?', (email, password)).fetchone()
    elif login_type == '3':  # Student
        user = conn.execute('SELECT * FROM student_list WHERE email = ? AND password = ?', (email, password)).fetchone()
...
def evaluate():
    ...
    if request.method == 'POST':
        try:
            ...
            academic_id = form_data['academic_id']
            class_id = form_data['class_id']
            student_id = session['login_id']
            subject_id = form_data['subject_id']
            faculty_id = form_data['faculty_id']
            restriction_id = form_data['restriction_id']
            question_ids = form_data.getlist('qid[]')
            ratings = form_data.getlist('rate[]')
            ...
            cursor = conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (academic_id, class_id, student_id, subject_id, faculty_id, restriction_id))
            ...
            for question_id, rating in zip(question_ids, ratings):
                print("Inserting:", evaluation_id, question_id, rating)
                conn.execute("""
                    INSERT INTO evaluation_answers (evaluation_id, question_id, rate)
                    VALUES (?, ?, ?)
                """, (evaluation_id, question_id, rating))
            ...
            conn.rollback()
            flash("An error occurred while saving data: " + str(e), "danger")
            print(f"Exception: {e}")
    ...
    academic_row = conn.execute("""SELECT * FROM academic_list""").fetchone()
    user_row = conn.execute("""SELECT * FROM student_list""").fetchone()
    
    session['academic'] = dict(academic_row) if academic_row else {}
    session['login_class_id'] = user_row['class_id'] if user_row else None
    ...
    restrictions = conn.execute("""
        ...
        WHERE r.academic_id = ? AND r.class_id = ? AND r.id NOT IN (
            SELECT restriction_id FROM evaluation_list 
            WHERE academic_id = ? AND student_id = ?
        )
    """, (session['academic']['id'], session['login_class_id'], session['academic']['id'], session['login_id'])).fetchall()
    ...
    criteria = conn.execute("""
        SELECT * 
        FROM criteria_list
        WHERE id IN (SELECT criteria_id FROM question_list WHERE academic_id = ?)
    ...
    return render_template('student/selectSubject.html', restrictions=restrictions, criteria=criteria, questions=questions, faculty_id=faculty_id, subject_id=subject_id, restriction_id=restriction_id)
...
def save_evaluation():
    ...
    try:
        ...
        for question_id, rating in zip(question_ids, ratings):
            conn.execute("""
                INSERT INTO evaluation_list (academic_id, class_id, student_id, subject_id, faculty_id, restriction_id, question_id, rating)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (academic_id, class_id, student_id, subject_id, faculty_id, restriction_id, question_id, rating))
...
@app.route('/ajax_save_evaluation', methods=['POST'])
def ajax_save_evaluation():
    if 'login_id' not in session or session.get('login_type') != '3':
        return "0"
    ...
    if request.method == 'POST':
        ...
        if id:
            query = '''
                ...
                WHERE id = ?
            '''
            conn.execute(query, (firstname, lastname, email, password, avatar, id))
        else:
            query = '''
                INSERT INTO users (firstname, lastname, email, password, avatar)
                VALUES (?, ?, ?, ?, ?)
            '''
            conn.execute(query, (firstname, lastname, email, password, avatar))
